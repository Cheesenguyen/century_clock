// ============================================================================
// TESTBENCH CHI TIET CHO THU MUC century_clock-main
...
module tb_counter;
    ...
    initial begin
        ...
        pulse_tick_n(180);  // tong 360 tick = 5 phut
        check(bcd_val(min_tens, min_ones), 5, "300 tick: phut=5");
        $display("");
        ...
        check(bcd_val(mon_tens, mon_ones), 1, "Thang giu nguyen: 1 (chinh doc lap)");
        ...
        check(bcd_val(mon_tens, mon_ones), 1, "Thang giu nguyen: 1");
        ...
        check(bcd_val(mon_tens, mon_ones), 4, "Thang giu nguyen: 4");
        ...
        check(bcd_val(mon_tens, mon_ones), 2, "Thang giu nguyen: 2");
        ...
        $display("");
                ...
                // ==============================================================
        // TEST 35: BUG NGAY/THANG INVALID - EDIT THANG LAM NGAY VUOT GIOI HAN
        // ==============================================================
        $display("--- TEST 35: Edit thang gay ngay invalid (31/1 -> chinh thang 2) ---");
        do_reset;

        // Dat ngay = 31 (thang 1, 31 ngay)
        set_edit(1'b1, 2'b01);  // DAY
        edit_up_n(30);           // ngay 1 -> 31
        clear_edit;
        check(bcd_val(day_tens, day_ones), 31, "Dat ngay = 31 (thang 1)");

        // Chinh THANG sang thang 2 (28 ngay, nam 2026 ko nhuan)
        // Luu y: edit_select dang chon THANG (SEL_2), khong phai NGAY (SEL_1)
        // nen up/down cua day_counter = 0 -> roi vao nhanh "khong tang khong giam"
        set_edit(1'b1, 2'b10);  // MONTH
        edit_up;                 // thang 1 -> 2
        clear_edit;
        check(bcd_val(mon_tens, mon_ones), 2, "Thang = 2");

        // Ngay phai tu dong duoc kep ve 28 (max thang 2, ko nhuan)
        // Neu FAIL (van con 31) -> bug ngay/thang chua duoc fix trong day_counter.v
        check(bcd_val(day_tens, day_ones), 28, "Ngay phai tu dong kep ve 28 (31/02 khong hop le)");
        $display("");

        // ==============================================================
        // TEST 36: BUG NGAY/NAM INVALID - EDIT NAM LAM NGAY 29/2 VUOT GIOI HAN
        // ==============================================================
        $display("--- TEST 36: Edit nam gay ngay invalid (29/2 nhuan -> chinh nam ko nhuan) ---");
        do_reset;
        ...
        // Dat ngay = 29 (hop le vi 2024 nhuan)
        set_edit(1'b1, 2'b01);  // DAY
        edit_up_n(28);            // ngay 1 -> 29
        clear_edit;
        check(bcd_val(day_tens, day_ones), 29, "Dat ngay = 29 (hop le, 2024 nhuan)");

        // Chinh NAM sang 2023 (ko nhuan) -> thang 2 chi con 28 ngay
        set_edit(1'b1, 2'b11);  // YEAR
        edit_down;                // 2024 -> 2023
        clear_edit;
        check(year_val(yr_thousands, yr_hundreds, yr_tens, yr_ones), 2023, "Nam = 2023 (ko nhuan)");

        // Ngay phai tu dong duoc kep ve 28
        // Neu FAIL (van con 29) -> bug ngay/nam chua duoc fix trong day_counter.v
        check(bcd_val(day_tens, day_ones), 28, "Ngay phai tu dong kep ve 28 (29/02 khong hop le nam 2023)");
        $display("");
